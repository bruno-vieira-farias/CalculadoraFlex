package br.com.onurbasfar.calculadoraflex.ui.result
...
class ResultActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (currentIntentExtra == null) {
            Toast.makeText(this, "Não foi possível realizar a operação", Toast.LENGTH_SHORT)
                .show()
        } else {
            calculate(currentIntentExtra)
        }
    }

    private fun calculate(intentExtras: Bundle) {
        val gasPrice = intentExtras.getDouble("GAS_PRICE", 0.0)
        val ethanolPrice = intentExtras.getDouble("ETHANOL_PRICE", 0.0)
        val gasAverage = intentExtras.getDouble("GAS_AVERAGE", 0.0)
        val ethanolAverage = intentExtras.getDouble("ETHANOL_AVERAGE", 0.0)
